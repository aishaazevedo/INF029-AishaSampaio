...
char *encontrar_trecho_no_nome(const char *nome_completo, const char *trecho_busca) {
    
    if (!*trecho_busca) return (char *)nome_completo;

    for (; *nome_completo; ++nome_completo) {
        
        if (para_maiuscula_manual(*nome_completo) == para_maiuscula_manual(*trecho_busca)) {
            
            const char *p_nome = nome_completo + 1;
            const char *p_busca = trecho_busca + 1;
            
            while (*p_nome && *p_busca) {
                if (para_maiuscula_manual(*p_nome) != para_maiuscula_manual(*p_busca)) {
                    break; 
                }
                ++p_nome;
                ++p_busca;
...
void buscarPessoasPorString(Matriculado vetorLocal[], int qtdPessoas, char busca[]) {
    ...
    for (int i = 0; i < qtdPessoas; i++) {
        if (vetorLocal[i].ativo == 1) {
            if (encontrar_trecho_no_nome(vetorLocal[i].nome, busca) != NULL) {
